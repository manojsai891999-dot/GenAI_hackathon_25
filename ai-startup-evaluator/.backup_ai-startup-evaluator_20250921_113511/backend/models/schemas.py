...
class InterviewResponse(Base):
    ...
    question_category = Column(String(100))  # problem, customers, traction, business_model, etc.
    response_timestamp = Column(DateTime(timezone=True), server_default=func.now())
    
    # Analysis fields
    sentiment_score = Column(Float)  # -1 to 1
    confidence_score = Column(Float)  # 0 to 1
    key_insights = Column(JSON)  # Array of insights extracted
    red_flags = Column(JSON)  # Array of red flags
    positive_signals = Column(JSON)  # Array of positive signals
...
class InterviewSession(Base):
    ...
    # GCS references
    summary_report_gcs_path = Column(String(500))
    transcript_gcs_path = Column(String(500))
